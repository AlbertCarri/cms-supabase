...
export const ShowMenu = ({ openEditModal, categoryId }) => {
  ...
  return (
    <div className="flex flex-col w-full">
      {/* Modal para eliminar menú con botones Aceptar / Cancelar */}
      {modal && (
      ...
      {menu.map((menus) => (
        <div
          key={menus.id}
          className="flex flex-col bg-stone-800 py-4 px-4 rounded-xl mb-4 shadow-2xl"
        >
          ...
          <div className="text-start mt-4">
            ...
            <div className="flex flex-row">
              {menus.alergens.map((alerg) => (
                <p
                  key={alerg}
                  className="label-emerald text-xs line-clamp-1 md:line-clamp-none rounded-sm px-1 mr-2 mb-2"
                >
                  {alerg}
                </p>
              ))}
          ...
          <div className="flex mx-auto mt-4">
            <button
              key={menus.id}
              className="button-red rounded-lg ml-10 w-24 h-10 mr-2"
              onClick={() => deleteConfirm(menus.id, menus.name)}
            >
              Eliminar
            </button>
            <button
              key={menus.id + "edit"}
              className="button-sky rounded-lg ml-2 w-24 h-10 mr-2"
              onClick={() => editMenu(menus.id)}
            >
              Editar
            </button>
            <input
